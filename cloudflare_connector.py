...
class CloudflareConnector(BaseConnector):
    ...
    def _flatten_response_data(self, response):
        try:
            ...
            response.update(response_data[0])
            del response['result']
        except:
            pass
    ...
    def _make_rest_call(self, endpoint, action_result, method="get", headers=None, **kwargs):
        ...
        self.save_progress("headers: {0}".format(headers))
        self.save_progress("url: {0}".format(url))
        try:
            r = request_func(
                            url,
                            headers=headers,  # basic authentication
    ...
    def _handle_create_filter(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call('/filters', action_result, json=filter_rule_data,
    ...
    def handle_action(self, param):
        ...
        elif action_id == 'update_filter':
            ret_val = self._handle_update_filter(param)
        ...
        elif action_id == 'unblock_ua':
            ret_val = self._handle_unblock_ua(param)

        elif action_id == 'block_ua':
            ret_val = self._handle_block_ua(param)
    ...
    def initialize(self):
        ...
        self._email = config.get('auth_email')
        self._service_key = config.get('service_key')
        self._zone_id = config.get('zone_id')
        self._base_url = "{0}/zones/{1}".format(config.get('api_url'), self._zone_id)
